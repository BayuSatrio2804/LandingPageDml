...
export default async function KarierPage() {
  const [COMPANY, { career }] = await Promise.all([getCompanyProfile(), getContactCareer()]);
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-4 py-16 md:px-8 md:py-24">
      ...
      <div className="mt-8 max-w-[60ch] rounded-card border border-surface-3 bg-surface-2 p-8">
        <p className="text-ink">{career.noOpeningsText}</p>
        <p className="mt-3 text-sm text-ink-muted">{career.spontaneousText}</p>
        <ExternalLink
          href={`https://wa.me/${COMPANY.whatsapp}?text=${encodeURIComponent(career.whatsappMessage)}`}
          label={career.whatsappButtonLabel}
